...
class ObjectCounter:
    ...
    def __init__(
        ...
        self.window_name = "YOLOv8 Dem Xe O To"
    ...
    def extract_and_process_tracks(self, tracks):
        ...
        if tracks[0].boxes.id is not None:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                ...
                if self.names[cls] not in self.class_wise_count:
                    self.class_wise_count[self.names[cls]] = {"Vao": 0, "Ra": 0}
                ...
                if len(self.reg_pts) >= 3:
                    ...
                    if prev_position is not None and is_inside and track_id not in self.count_ids:
                        ...
                        if (box[0] - prev_position[0]) * (self.counting_region.centroid.x - prev_position[0]) > 0:
                            self.in_counts += 1
                            self.class_wise_count[self.names[cls]]["Vao"] += 1
                        else:
                            self.out_counts += 1
                            self.class_wise_count[self.names[cls]]["Ra"] += 1
        ...
        for key, value in self.class_wise_count.items():
            if value["Vao"] != 0 or value["Ra"] != 0:
                if not self.view_in_counts and not self.view_out_counts:
                    continue
                elif not self.view_in_counts:
                    labels_dict[str.capitalize(key)] = f"Ra {value['Ra']}"
                elif not self.view_out_counts:
                    labels_dict[str.capitalize(key)] = f"Vao {value['IN']}"
                else:
                    labels_dict[str.capitalize(key)] = f"Vao {value['IN']} Ra {value['Ra']}"
...
if __name__ == "__main__":
    classes_names = {0: "car"}  # example class names
